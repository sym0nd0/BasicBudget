...
interface SavingsContextValue {
  ...
  createTransaction: (goalId: string, data: { type: string; amount_pence: number; notes?: string | null }) => Promise<SavingsTransaction>;
  refetchGoals: () => void;
}
...
export function SavingsProvider({ children }: { children: React.ReactNode }) {
  ...
  const createTransaction = useCallback(async (goalId: string, data: { type: string; amount_pence: number; notes?: string | null }) => {
